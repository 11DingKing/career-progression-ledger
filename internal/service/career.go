...
func (s *CareerService) AddEmployment(ctx context.Context, e domain.EmploymentRecord) error {
	...
	e.CreatedAt = now
	e.UpdatedAt = now
	if err := s.DB.Tx(ctx, func(tx *sql.Tx) error { return s.Repo.CreateEmployment(ctx, tx, e) }); err != nil {
		return err
	}
	if e.Status == domain.EmploymentActive {
		n, err := s.Repo.CountActiveEmployment(ctx, e.GraduateID, e.StartDate)
		if err != nil {
			return err
		}
		if n > 1 {
